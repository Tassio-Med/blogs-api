...
const infixPost = async (req, res) => {
  ...
  const { type, message } = await blogPostsServices.insertBlogPost({ userId, ...blogPost });
...
const catchPost = async (_req, res) => {
  const message = await blogPostsServices.catchPost();
  res.status(OK).json(message);
};

const getPostById = async (req, res) => {
  const { id } = req.params;
  const { type, message } = await blogPostsServices.getPostById(id);
